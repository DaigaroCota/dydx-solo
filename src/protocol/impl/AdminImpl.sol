...
library AdminImpl {
  ...
  function ownerSetIsClosing(Storage.State storage state, uint256 marketId, bool isClosing) public {
    _validateMarketId(state, marketId);
  ...
  function ownerSetMarginRatio(Storage.State storage state, Decimal.D256 memory ratio) public {
    Require.that(ratio.value <= state.riskLimits.marginRatioMax, FILE, "Ratio too high");
    Require.that(
      ratio.value > state.riskParams.liquidationSpread.value, FILE, "Ratio cannot be <= spread"
    );
  ...
  function ownerSetLiquidationSpread(
    ...
    public
  {
    Require.that(spread.value <= state.riskLimits.liquidationSpreadMax, FILE, "Spread too high");
    Require.that(
      spread.value < state.riskParams.marginRatio.value, FILE, "Spread cannot be >= ratio"
    );
  ...
  function ownerSetEarningsRate(
    ...
    public
  {
    Require.that(earningsRate.value <= state.riskLimits.earningsRateMax, FILE, "Rate too high");
  ...
  {
    Require.that(
      minBorrowedValue.value <= state.riskLimits.minBorrowedValueMax, FILE, "Value too high"
    );
    ...
    private
  {
    ...
    Require.that(priceOracle.getPrice(token).value != 0, FILE, "Invalid oracle price");
  ...
  {
    Require.that(
      marginPremium.value <= state.riskLimits.marginPremiumMax, FILE, "Margin premium too high"
    );
  ...
  {
    Require.that(
      spreadPremium.value <= state.riskLimits.spreadPremiumMax, FILE, "Spread premium too high"
    );
  ...
  function _requireNoMarket(Storage.State storage state, address token) private view {
  ...
  function _validateMarketId(Storage.State storage state, uint256 marketId) private view {
