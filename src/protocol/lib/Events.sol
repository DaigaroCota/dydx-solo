...
library Events {
  ...
  event LogCall(address indexed accountOwner, uint256 accountNumber, address callee);
  ...
  function logIndexUpdate(uint256 marketId, Interest.Index memory index) internal {
    ...
    internal
  {
    emit LogTransfer(
      ...
      getBalanceUpdate(state, args.accountTwo, args.market, deltaWei.negative())
    );
  }
    ...
    internal
  {
    BalanceUpdate[4] memory updates = [
      getBalanceUpdate(state, args.takerAccount, args.inputMarket, inputWei.negative()),
      getBalanceUpdate(state, args.takerAccount, args.outputMarket, outputWei.negative()),
      getBalanceUpdate(state, args.makerAccount, args.inputMarket, inputWei),
      getBalanceUpdate(state, args.makerAccount, args.outputMarket, outputWei)
    ];
  ...
  function logLiquidate(
    ...
    internal
  {
    BalanceUpdate memory solidHeldUpdate =
      getBalanceUpdate(state, args.solidAccount, args.heldMarket, heldWei.negative());
    BalanceUpdate memory solidOwedUpdate =
      getBalanceUpdate(state, args.solidAccount, args.owedMarket, owedWei.negative());
    BalanceUpdate memory liquidHeldUpdate =
      getBalanceUpdate(state, args.liquidAccount, args.heldMarket, heldWei);
    BalanceUpdate memory liquidOwedUpdate =
      getBalanceUpdate(state, args.liquidAccount, args.owedMarket, owedWei);
    ...
    internal
  {
    ...
    BalanceUpdate memory solidOwedUpdate =
      getBalanceUpdate(state, args.solidAccount, args.owedMarket, owedWei.negative());
    BalanceUpdate memory vaporOwedUpdate =
      getBalanceUpdate(state, args.vaporAccount, args.owedMarket, owedWei.add(excessWei));
  ...
  {
    return BalanceUpdate({deltaWei: deltaWei, newPar: state.getPar(account, market)});
  }
}
