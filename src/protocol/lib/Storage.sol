...
library Storage {
  ...
  function getToken(Storage.State storage state, uint256 marketId) internal view returns (address) {
    ...
    returns (Types.Wei memory)
  {
    ...
    Types.Wei memory balanceWei =
      Types.Wei({sign: true, value: Token.balanceOf(token, address(this))});

    (Types.Wei memory supplyWei, Types.Wei memory borrowWei) =
    ...
    returns (Interest.Index memory)
  {
    ...
    return Interest.calculateNewIndex(
      index, rate, state.getTotalPar(marketId), state.riskParams.earningsRate
    );
  }
    ...
    returns (Interest.Rate memory)
  {
    ...
    Interest.Rate memory rate = state.markets[marketId].interestSetter.getInterestRate(
      state.getToken(marketId), borrowWei.value, supplyWei.value
    );
  ...
  function requireIsOperator(
    ...
    view
  {
    bool isValidOperator = operator == account.owner || state.isGlobalOperator(operator)
      || state.isLocalOperator(account.owner, operator);

    Require.that(isValidOperator, FILE, "Unpermissioned operator", operator);
  }
    ...
    returns (Types.Par memory, Types.Wei memory)
  {
    ...
    if (amount.denomination == Types.AssetDenomination.Wei) {
      ...
      newPar = Types.Par({sign: amount.sign, value: amount.value.to128()});
    ...
    returns (Types.Par memory, Types.Wei memory)
  {
    ...
    (Types.Par memory newPar, Types.Wei memory deltaWei) =
      state.getNewParAndDeltaWei(account, marketId, amount);
