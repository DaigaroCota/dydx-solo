 ...
 */
contract DaiPriceOracle is Ownable, IPriceOracle {
  ...
  event PriceSet(PriceInfo newPriceInfo);
    ...
    public
  {
    ...
    g_priceInfo = PriceInfo({lastUpdate: uint32(block.timestamp), price: uint128(EXPECTED_PRICE)});
  }

  // ============ Admin Functions ============

  function ownerSetPokerAddress(address newPoker) external onlyOwner {
    g_poker = newPoker;
  }
  ...
  {
    Require.that(msg.sender == g_poker, FILE, "Only poker can call updatePrice", msg.sender);

    Monetary.Price memory newPrice = getBoundedTargetPrice();

    Require.that(
      newPrice.value >= minimum.value, FILE, "newPrice below minimum", newPrice.value, minimum.value
    );

    Require.that(
      newPrice.value <= maximum.value, FILE, "newPrice above maximum", newPrice.value, maximum.value
    );

    g_priceInfo = PriceInfo({price: Math.to128(newPrice.value), lastUpdate: Time.currentTime()});
  ...
  function getPrice(address /* token */ ) public view returns (Monetary.Price memory) {
    return Monetary.Price({value: g_priceInfo.price});
  }
   ...
   */
  function getBoundedTargetPrice() public view returns (Monetary.Price memory) {
    ...
    return Monetary.Price({value: boundedTargetPrice});
  }
   ...
   */
  function getTargetPrice() public view returns (Monetary.Price memory) {
    uint256 targetPrice = getMidValue(EXPECTED_PRICE, getCurvePrice(), getUniswapPrice());

    return Monetary.Price({value: targetPrice});
  }
  ...
  function getCurvePrice() public view returns (uint256) {
    ...
    uint256 dyWithoutFee = dyWithFee.mul(CURVE_FEE_DENOMINATOR).div(CURVE_FEE_DENOMINATOR.sub(fee));
  ...
  function getUniswapPrice() public view returns (uint256) {
    ...
    return UNISWAP_DECIMALS_BASE.mul(usdcAmt).mul(poolOneEthAmt).div(poolTwoEthAmt).div(daiAmt);
  }
    ...
    returns (uint256, uint256)
  {
    ...
    return (oldPrice.sub(maxDeviation), oldPrice.add(maxDeviation));
  }
