 ...
 */
contract PolynomialInterestSetter is IInterestSetter {
  ...
  constructor(PolyStorage memory params) public {
    ...
    returns (Interest.Rate memory)
  {
    ...
    return Interest.Rate({value: result * maxAPR / (SECONDS_IN_A_YEAR * BASE * PERCENT)});
  }
  ...
  function getCoefficients() external view returns (uint256[] memory) {
    ...
    for (uint256 coefficients = g_storage.coefficients; coefficients != 0; coefficients >>= BYTE) {
