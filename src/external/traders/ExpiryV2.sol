 ...
 */
contract ExpiryV2 is Ownable, OnlySolo, ICallee, IAutoTrader {
  ...
  event LogSenderApproved(address approver, address sender, uint32 minTimeDelta);
  ...
  function callFunctionSetExpiry(address sender, bytes memory data) private {
    (CallFunctionType callType, SetExpiryArg[] memory expiries) =
    ...
    for (uint256 i = 0; i < expiries.length; i++) {
      ...
      if (exp.timeDelta != 0 && SOLO_MARGIN.getAccountPar(exp.account, exp.marketId).isNegative()) {
  ...
  function callFunctionSetApproval(address sender, bytes memory data) private {
    (CallFunctionType callType, SetApprovalArg memory approvalArg) =
  ...
  function setApproval(address approver, address sender, uint32 minTimeDelta) private {
