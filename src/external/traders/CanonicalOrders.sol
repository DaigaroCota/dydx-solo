 ...
 */
contract CanonicalOrders is Ownable, OnlySolo, IAutoTrader, ICallee {
  ...
  bytes32 private constant FILE = "CanonicalOrders";

  // EIP191 header for EIP712 prefix
  bytes2 private constant EIP191_HEADER = 0x1901;

  // EIP712 Domain Name value
  string private constant EIP712_DOMAIN_NAME = "CanonicalOrders";
  ...
  bytes32 private constant EIP712_DOMAIN_SEPARATOR_SCHEMA_HASH = keccak256(
    abi.encodePacked(
      "EIP712Domain(",
      "string name,",
      "string version,",
  ...
  /* solium-disable-next-line indentation */
  bytes32 private constant EIP712_ORDER_STRUCT_SCHEMA_HASH = keccak256(
    abi.encodePacked(
      "CanonicalOrder(",
      "bytes32 flags,",
      "uint256 baseMarket,",
  ...
  uint256 private constant NUM_ORDER_AND_FILL_BYTES = 416;
  ...
  uint256 private constant PRICE_BASE = 10 ** 18;

  // Bitmasks for the order.flag argument
  bytes32 private constant IS_BUY_FLAG = bytes32(uint256(1));
  bytes32 private constant IS_DECREASE_ONLY_FLAG = bytes32(uint256(1 << 1));
  bytes32 private constant IS_NEGATIVE_FEE_FLAG = bytes32(uint256(1 << 2));
  ...
  event LogContractStatusSet(bool operational);

  event LogTakerSet(address taker);

  event LogCanonicalOrderCanceled(
    bytes32 indexed orderHash, address indexed canceler, uint256 baseMarket, uint256 quoteMarket
  );

  event LogCanonicalOrderApproved(
    bytes32 indexed orderHash, address indexed approver, uint256 baseMarket, uint256 quoteMarket
  );
  ...
  constructor(address soloMargin, address taker, uint256 chainId) public OnlySolo(soloMargin) {
   ...
   */
  function setTakerAddress(address taker) external onlyOwner {
    g_taker = taker;
  ...
  {
    Require.that(g_isOperational, FILE, "Contract is not operational");

    OrderInfo memory orderInfo = getOrderInfo(data);

    verifySignature(orderInfo, data);

    verifyOrderInfo(orderInfo, makerAccount, takerAccount, inputMarketId, outputMarketId, inputWei);

    Types.AssetAmount memory assetAmount =
      getOutputAssetAmount(inputMarketId, outputMarketId, inputWei, orderInfo);

    if (isDecreaseOnly(orderInfo.order)) {
      verifyDecreaseOnly(oldInputPar, newInputPar, assetAmount, makerAccount, outputMarketId);
    }
  ...
  function cancelOrderInternal(address canceler, Order memory order) private {
    Require.that(canceler == order.makerAccountOwner, FILE, "Canceler must be maker");
    bytes32 orderHash = getOrderHash(order);
    g_status[orderHash] = OrderStatus.Canceled;
    emit LogCanonicalOrderCanceled(orderHash, canceler, order.baseMarket, order.quoteMarket);
  }

  /**
   * Approves an order as long as it is not already approved or canceled.
   */
  function approveOrderInternal(address approver, Order memory order) private {
    Require.that(approver == order.makerAccountOwner, FILE, "Approver must be maker");
    bytes32 orderHash = getOrderHash(order);
    Require.that(
      g_status[orderHash] != OrderStatus.Canceled, FILE, "Cannot approve canceled order", orderHash
    );
    g_status[orderHash] = OrderStatus.Approved;
    emit LogCanonicalOrderApproved(orderHash, approver, order.baseMarket, order.quoteMarket);
  }
   ...
   */
  function getOrderInfo(bytes memory data) private returns (OrderInfo memory) {
    Require.that(
      (
    ...
    (orderInfo.order, orderInfo.fill) = abi.decode(data, (Order, FillArgs));

    // load fillArgs from storage if price is zero
    if (orderInfo.fill.price == 0) {
      orderInfo.fill = g_fillArgs;
      g_fillArgs = FillArgs({price: 0, fee: 0, isNegativeFee: false});
    }
    Require.that(orderInfo.fill.price != 0, FILE, "FillArgs loaded price is zero");
  ...
  function verifySignature(OrderInfo memory orderInfo, bytes memory data) private view {
    ...
    if (orderStatus == OrderStatus.Null) {
      ...
      Require.that(orderStatus != OrderStatus.Canceled, FILE, "Order canceled", orderInfo.orderHash);
    ...
    view
  {
    ...
    bool validPrice = isBuy(orderInfo.order)
      ...
      : fill.price >= orderInfo.order.limitPrice;
    Require.that(validPrice, FILE, "Fill invalid price");

    // verify fill fee
    bool validFee = isNegativeLimitFee(orderInfo.order)
      ? (fill.fee >= orderInfo.order.limitFee) && fill.isNegativeFee
      : (fill.fee <= orderInfo.order.limitFee) || fill.isNegativeFee;
    Require.that(validFee, FILE, "Fill invalid fee");

    // verify triggerPrice
    if (orderInfo.order.triggerPrice > 0) {
      uint256 currentPrice =
        getCurrentPrice(orderInfo.order.baseMarket, orderInfo.order.quoteMarket);
      Require.that(
    ...
    // verify taker
    Require.that(takerAccount.owner == g_taker, FILE, "Order taker mismatch", orderInfo.orderHash);

    // verify markets
    Require.that(
      (orderInfo.order.baseMarket == outputMarketId && orderInfo.order.quoteMarket == inputMarketId)
        || (
          orderInfo.order.quoteMarket == outputMarketId && orderInfo.order.baseMarket == inputMarketId
        ),
      FILE,
      "Market mismatch",
      orderInfo.orderHash
    ...
    Require.that(!inputWei.isZero(), FILE, "InputWei is zero", orderInfo.orderHash);
    ...
    Require.that(
      inputWei.sign == ((orderInfo.order.baseMarket == inputMarketId) == isBuy(orderInfo.order)),
      FILE,
   ...
   */
  function updateFilledAmount(OrderInfo memory orderInfo, uint256 fillAmount) private {
   ...
   */
  function getCurrentPrice(uint256 baseMarket, uint256 quoteMarket) private view returns (uint256) {
  ...
  function getOrderHash(Order memory order) private view returns (bytes32) {
    ...
    /* solium-disable-next-line indentation */
    bytes32 structHash = keccak256(abi.encode(EIP712_ORDER_STRUCT_SCHEMA_HASH, order));

    // compute eip712 compliant hash
    /* solium-disable-next-line indentation */
    return keccak256(abi.encodePacked(EIP191_HEADER, EIP712_DOMAIN_HASH, structHash));
  }

  /**
   * Parses out a signature from call data.
   */
  function parseSignature(bytes memory data) private pure returns (bytes memory) {
    Require.that(
      data.length == NUM_ORDER_AND_FILL_BYTES + NUM_SIGNATURE_BYTES,
      FILE,
   ...
   */
  function isBuy(Order memory order) private pure returns (bool) {
   ...
   */
  function isDecreaseOnly(Order memory order) private pure returns (bool) {
   ...
   */
  function isNegativeLimitFee(Order memory order) private pure returns (bool) {
