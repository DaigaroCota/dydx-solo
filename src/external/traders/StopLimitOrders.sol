 ...
 */
contract StopLimitOrders is Ownable, OnlySolo, IAutoTrader, ICallee {
  ...
  bytes32 private constant FILE = "StopLimitOrders";
  ...
  event LogStopLimitOrderCanceled(
    bytes32 indexed orderHash, address indexed canceler, uint256 makerMarket, uint256 takerMarket
  );

  event LogStopLimitOrderApproved(
    bytes32 indexed orderHash, address indexed approver, uint256 makerMarket, uint256 takerMarket
  );
  ...
  constructor(address soloMargin, uint256 chainId) public OnlySolo(soloMargin) {
    ...
    returns (Types.AssetAmount memory)
  {
    ...
    verifyOrderAndAccountsAndMarkets(
      orderInfo, makerAccount, takerAccount, inputMarketId, outputMarketId, inputWei
    );
  ...
  function callFunction(
    ...
    onlySolo(msg.sender)
  {
    Require.that(data.length == NUM_CALLFUNCTIONDATA_BYTES, FILE, "Cannot parse CallFunctionData");
  ...
  function cancelOrderInternal(address canceler, Order memory order) private {
    ...
    emit LogStopLimitOrderCanceled(orderHash, canceler, order.makerMarket, order.takerMarket);
  }
  ...
  function approveOrderInternal(address approver, Order memory order) private {
    ...
    emit LogStopLimitOrderApproved(orderHash, approver, order.makerMarket, order.takerMarket);
  }
    ...
    view
  {
    ...
    if (orderInfo.order.triggerPrice > 0) {
      uint256 currentPrice =
        getCurrentPrice(orderInfo.order.makerMarket, orderInfo.order.takerMarket);
      Require.that(
    ...
    returns (Types.AssetAmount memory)
  {
    ...
    if (orderInfo.order.takerMarket == inputMarketId) {
      outputAmount =
        inputWei.value.getPartial(orderInfo.order.makerAmount, orderInfo.order.takerAmount);
      makerFillAmount = outputAmount;
    } else {
      assert(orderInfo.order.takerMarket == outputMarketId);
      outputAmount =
        inputWei.value.getPartialRoundUp(orderInfo.order.takerAmount, orderInfo.order.makerAmount);
   ...
   */
  function updateMakerFilledAmount(OrderInfo memory orderInfo, uint256 makerFillAmount) private {
   ...
   */
  function getCurrentPrice(uint256 makerMarket, uint256 takerMarket) private view returns (uint256) {
   ...
   */
  function getOrderAndValidateSignature(bytes memory data) private view returns (OrderInfo memory) {
    Require.that(
      (data.length == NUM_ORDER_BYTES || data.length == NUM_ORDER_BYTES + NUM_SIGNATURE_BYTES),
      FILE,
  ...
  function parseSignature(bytes memory data) private pure returns (bytes memory) {
    Require.that(
      data.length == NUM_ORDER_BYTES + NUM_SIGNATURE_BYTES, FILE, "Cannot parse signature from data"
    );
