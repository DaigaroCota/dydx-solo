 ...
 */
contract LimitOrders is Ownable, OnlySolo, IAutoTrader, ICallee {
  ...
  bytes32 private constant FILE = "LimitOrders";

  // EIP191 header for EIP712 prefix
  bytes2 private constant EIP191_HEADER = 0x1901;

  // EIP712 Domain Name value
  string private constant EIP712_DOMAIN_NAME = "LimitOrders";
  ...
  /* solium-disable-next-line indentation */
  bytes32 private constant EIP712_LIMIT_ORDER_STRUCT_SCHEMA_HASH = keccak256(
    abi.encodePacked(
      "LimitOrder(",
      "uint256 makerMarket,",
      "uint256 takerMarket,",
  ...
  uint256 private constant NUM_ORDER_BYTES = 320;
    ...
    returns (Types.AssetAmount memory)
  {
    ...
    return getOutputAssetAmount(inputMarketId, outputMarketId, inputWei, orderInfo);
  }
  ...
  function cancelOrderInternal(address canceler, Order memory order) private {
    ...
    emit LogLimitOrderCanceled(orderHash, canceler, order.makerMarket, order.takerMarket);
  }
  ...
  function approveOrderInternal(address approver, Order memory order) private {
    ...
    emit LogLimitOrderApproved(orderHash, approver, order.makerMarket, order.takerMarket);
  }
  ...
  function getOrderHash(Order memory order) private view returns (bytes32) {
    ...
    /* solium-disable-next-line indentation */
    bytes32 structHash = keccak256(abi.encode(EIP712_LIMIT_ORDER_STRUCT_SCHEMA_HASH, order));
