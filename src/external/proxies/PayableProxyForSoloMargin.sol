 ...
 */
contract PayableProxyForSoloMargin is OnlySolo, ReentrancyGuard {
  // ============ Constants ============
  ...
  constructor(address soloMargin, address payable weth) public OnlySolo(soloMargin) {
    WETH = WETH9(weth);
  ...
  function() external payable {
    ...
    msg.sender == address(WETH), "Cannot receive ETH");
  }
    ...
    nonReentrant
  {
    ...
    for (uint256 i = 0; i < actions.length; i++) {
      ...
      Require.that(owner1 == msg.sender, FILE, "Sender must be primary account", owner1);
      ...
      if (action.actionType == Actions.ActionType.Transfer) {
        ...
        Require.that(owner2 == msg.sender, FILE, "Sender must be secondary account", owner2);
      }
    }
    ...
    if (remainingWeth != 0) {
      Require.that(sendEthTo != address(0), FILE, "Must set sendEthTo");
