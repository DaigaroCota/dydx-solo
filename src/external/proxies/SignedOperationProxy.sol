 ...
 */
contract SignedOperationProxy is OnlySolo, Ownable {
  using SafeMath for uint256;

  // ============ Constants ============

  bytes32 private constant FILE = "SignedOperationProxy";

  // EIP191 header for EIP712 prefix
  bytes2 private constant EIP191_HEADER = 0x1901;

  // EIP712 Domain Name value
  string private constant EIP712_DOMAIN_NAME = "SignedOperationProxy";

  // EIP712 Domain Version value
  string private constant EIP712_DOMAIN_VERSION = "1.1";

  // EIP712 encodeType of EIP712Domain
  bytes private constant EIP712_DOMAIN_STRING = abi.encodePacked(
    "EIP712Domain(",
    "string name,",
    "string version,",
    "uint256 chainId,",
  ...
  bytes private constant EIP712_OPERATION_STRING = abi.encodePacked(
    "Operation(",
    "Action[] actions,",
    "uint256 expiration,",
    "uint256 salt,",
  ...
  bytes private constant EIP712_ACTION_STRING = abi.encodePacked(
    "Action(",
    "uint8 actionType,",
  ...
  bytes private constant EIP712_ASSET_AMOUNT_STRING = abi.encodePacked(
    "AssetAmount(", "bool sign,", "uint8 denomination,", "uint8 ref,", "uint256 value", ")"
  );

  // EIP712 typeHash of EIP712Domain
  /* solium-disable-next-line indentation */
  bytes32 private constant EIP712_DOMAIN_SEPARATOR_SCHEMA_HASH =
    keccak256(abi.encodePacked(EIP712_DOMAIN_STRING));

  // EIP712 typeHash of Operation
  /* solium-disable-next-line indentation */
  bytes32 private constant EIP712_OPERATION_HASH = keccak256(
    abi.encodePacked(EIP712_OPERATION_STRING, EIP712_ACTION_STRING, EIP712_ASSET_AMOUNT_STRING)
  );

  // EIP712 typeHash of Action
  /* solium-disable-next-line indentation */
  bytes32 private constant EIP712_ACTION_HASH =
    keccak256(abi.encodePacked(EIP712_ACTION_STRING, EIP712_ASSET_AMOUNT_STRING));

  // EIP712 typeHash of AssetAmount
  /* solium-disable-next-line indentation */
  bytes32 private constant EIP712_ASSET_AMOUNT_HASH =
    keccak256(abi.encodePacked(EIP712_ASSET_AMOUNT_STRING));
  ...
  event LogOperationExecuted(
    bytes32 indexed operationHash, address indexed signer, address indexed sender
  );

  event LogOperationCanceled(bytes32 indexed operationHash, address indexed canceler);
  ...
  function cancel(
    ...
    public
  {
    bytes32 operationHash = getOperationHash(accounts, actions, auth, 0);
    Require.that(auth.header.signer == msg.sender, FILE, "Canceler must be signer");
    ...
    public
  {
    ...
    for (uint256 authIdx = 0; authIdx < auths.length; authIdx++) {
      ...
      if (auth.signature.length != 0) {
        ...
        bytes32 operationHash = getOperationHash(accounts, actions, auth, actionStartIdx);

        // require that this message is still valid
        Require.that(
          !g_invalidated[operationHash], FILE, "Hash already used or canceled", operationHash
        );
        ...
        Require.that(auth.header.signer == signer, FILE, "Invalid signature");
    ...
    Require.that(actionStartIdx == actions.length, FILE, "Not all actions are signed");
   ...
   * previously executed).
   */
  function getOperationsAreInvalid(bytes32[] memory operationHashes)
    public
    view
    returns (bool[] memory)
  {
   ...
   */
  function validateAccountOwner(address accountOwner, address signer) private view {
    bool valid = msg.sender == accountOwner || signer == accountOwner
      || SOLO_MARGIN.getIsLocalOperator(accountOwner, msg.sender)
      || SOLO_MARGIN.getIsLocalOperator(accountOwner, signer);

    Require.that(valid, FILE, "Signer not authorized", signer);
  }
    ...
    returns (bytes32)
  {
    ...
    bytes32 actionsEncoding = getActionsEncoding(accounts, actions, auth, startIdx);

    // compute the EIP712 hashStruct of an Operation struct
    /* solium-disable-next-line indentation */
    bytes32 structHash = keccak256(abi.encode(EIP712_OPERATION_HASH, actionsEncoding, auth.header));
    ...
    returns (bytes32)
  {
    ...
    for (uint256 i = 0; i < auth.numActions; i++) {
      ...
      ) ? Account.Info({owner: address(0), number: 0}) : accounts[action.otherAccountId];

      // compute the individual hash for the action
      /* solium-disable-next-line indentation */
      actionsBytes[i] = getActionHash(action, accounts[action.accountId], otherAccount);
    }
   ...
   */
  function getAssetAmountHash(Types.AssetAmount memory amount) private pure returns (bytes32) {
    /* solium-disable-next-line indentation */
    return keccak256(
      abi.encode(
        EIP712_ASSET_AMOUNT_HASH, amount.sign, amount.denomination, amount.ref, amount.value
      )
    );
  }
}
