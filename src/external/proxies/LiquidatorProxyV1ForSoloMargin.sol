 ...
 */
contract LiquidatorProxyV1ForSoloMargin is OnlySolo, ReentrancyGuard {
  ...
  constructor(address soloMargin) public OnlySolo(soloMargin) {} /* solium-disable-line no-empty-blocks */
    ...
    nonReentrant
  {
    ...
    for (uint256 owedIndex = 0; owedIndex < owedPreferences.length; owedIndex++) {
      ...
      for (uint256 heldIndex = 0; heldIndex < heldPreferences.length; heldIndex++) {
        ...
        Cache memory cache = initializeCache(constants, heldMarket, owedMarket);
        ...
        SOLO_MARGIN.operate(constructAccountsArray(constants), constructActionsArray(cache));
   ...
   */
  function calculateSafeLiquidationAmount(Cache memory cache) private pure {
    ...
    if (negOwed) {
      ...
      owedGoesToZeroLast =
        cache.owedWei.value.mul(cache.owedPriceAdj) > cache.heldWei.value.mul(cache.heldPrice);
    }
    ...
    if (owedGoesToZeroLast) {
      ...
      setCacheWeiValues(cache, cache.heldWei.add(heldWeiDelta), Types.zeroWei());
    } else {
      ...
      setCacheWeiValues(cache, Types.zeroWei(), cache.owedWei.add(owedWeiDelta));
    }
  }
    ...
    pure
  {
    ...
    bool liquidatorAboveCollateralization =
      isCollateralized(cache.supplyValue, cache.borrowValue, constants.minLiquidatorRatio);
    ...
    Decimal.D256 memory spreadMarginDiff =
      Decimal.D256({value: constants.minLiquidatorRatio.value.sub(cache.spread.value)});
   ...
   */
  function checkRequirements(Constants memory constants) private view {
    ...
    (Monetary.Value memory liquidSupplyValue, Monetary.Value memory liquidBorrowValue) =
      getCurrentAccountValues(constants, constants.liquidAccount);
    Require.that(liquidSupplyValue.value != 0, FILE, "Liquid account no supply");
    Require.that(
      SOLO_MARGIN.getAccountStatus(constants.liquidAccount) == Account.Status.Liquid
        || !isCollateralized(
          liquidSupplyValue.value, liquidBorrowValue.value, SOLO_MARGIN.getMarginRatio()
        ),
      FILE,
   ...
   */
  function getMarketsInfo() private view returns (MarketInfo[] memory) {
  ...
  {
    (Monetary.Value memory supplyValue, Monetary.Value memory borrowValue) =
  ...
  function constructAccountsArray(Constants memory constants)
    private
    pure
  ...
  function constructActionsArray(Cache memory cache)
    private
    pure
