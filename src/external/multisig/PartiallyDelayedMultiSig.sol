 ...
 */
contract PartiallyDelayedMultiSig is DelayedMultiSig {
  // ============ Events ============
  ...
  bytes4 internal constant BYTES_ZERO = bytes4(0x0);
  ...
  {
    require(
      _noDelayDestinations.length == _noDelaySelectors.length, "ADDRESS_AND_SELECTOR_MISMATCH"
    );
   ...
   */
  function setSelector(address destination, bytes4 selector, bool approved) public onlyWallet {
   ...
   */
  function txCanBeExecutedInstantly(uint256 transactionId) internal view returns (bool) {
  ...
  function selectorCanBeExecutedInstantly(
    ...
    returns (bool)
  {
    return instantData[destination][selector] || instantData[ADDRESS_ZERO][selector];
  }
}
